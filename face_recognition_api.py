...
def load_image_file(filename, mode='RGB'):
    ...
    if img.shape[0] > 800:
        baseheight = 500.0
        baseheight_int=500
        w = float(baseheight / img.shape[0])
        p = int(img.shape[1] * w)
        
        #print p,w,500.0/img.shape[0]
        #img = scipy.misc.imresize(img, (baseheight_int, p))
        # zoom
        img = cv2.resize(img, (p,baseheight_int), interpolation=cv2.INTER_AREA)

        #img=scipy.misc.imresize(img, (baseheight_int, p), interp='nearest')
        ...
        #img=scipy.misc.imresize(img, (baseheight_int, p), interp='nearest')
    #print img.shape
    #gray =  np.round(scipy.misc.imresize(img, (baseheight_int, p), interp='nearest') / 255.0)#cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
